...
class BaseServices:
    ...
    async def get_by_id(self, _id, fields_limit: list | str = None, ignore_error: bool = False, include_deleted: bool = False) -> dict:
        """
        ...
        if not item and not ignore_error:
            raise ValueError(f"{self.service_name} not found with id: {_id}")
        return item
    ...
    async def get_by_field(self, data: str, field_name: str, fields_limit: list | str = None, ignore_error: bool = False, include_deleted: bool = False) -> dict:
        ...
        if not item and not ignore_error:
            raise ValueError(f"{self.service_name} not found with {field_name}: {data}")
        return item
    ...
    async def _check_modified(self, old_data: dict, new_data: dict, ignore_error: bool) -> bool:
        ...
        if ignore_error:
            return False
        raise NameError("Data not modified")
    ...
    async def _check_unique(self, data: dict, unique_field: str | list, ignore_error: bool = False) -> bool:
        ...
        raise ValueError(f"{self.service_name} already exists with {first_item}")
    ...
    async def save_unique(self, data: dict, unique_field: str | list, ignore_error: bool = False) -> dict:
        ...
        if not item and not ignore_error:
            ...
            raise ValueError(f"{self.service_name} already exists with {unique_field}: {unique_value}")
        result = await self.get_by_id(_id=item)
        return result
    ...
    async def hard_delete_by_id(self, _id: str, ignore_error: bool = False, include_deleted: bool = False) -> bool:
        """
        ...
        """
        self.ensure_crud_provided()
        await self.get_by_id(_id=_id, ignore_error=ignore_error, include_deleted=include_deleted)
        result = await self.crud.delete_by_id(_id=_id)
        if not result:
            raise ValueError(f"{self.service_name} not found with id: {_id}")
        return result

    async def soft_delete_by_id(self, _id: str, ignore_error: bool = False) -> dict:
        """
        ...
        result = await self.update_by_id(_id=_id, data=data_update, ignore_error=ignore_error)
        return result
